...
//
//   // ✅ FIXED: Define status options directly
...
//       _storeController.setEditMode(widget.store!);
...
class _EditStoreScreenState extends State<EditStoreScreen>
  ...
  // ✅ NEW: Separate method to populate form from store data
  void _populateFormFromStore(StoreModel store) {
    ...
    // ✅ FIXED: Populate all form fields with null safety
    _storeController.ownerNameController.text =
        store.user?.name ?? store.ownerName;
    _storeController.ownerEmailController.text =
        store.user?.email ?? store.ownerEmail;
    _storeController.ownerPhoneController.text =
        store.user?.phone ?? store.phone;
    ...
    if (store.hasImage) {
      final imageUrl = store.fullImageUrl;
      _selectedImageBase64 = imageUrl;
      _storeController.selectedImageBase64.value = imageUrl;
    } else {
      _selectedImageBase64 = null;
    ...
    print('   - Has Image: ${_selectedImageBase64 != null}');
  }
